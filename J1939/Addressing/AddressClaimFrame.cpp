...
AddressClaimFrame::AddressClaimFrame(EcuName name)
	: J1939Frame(ADDRESS_CLAIM_PGN), mEcuName(name)
{
	mName = ADDRESS_CLAIM_NAME;
}
...
void AddressClaimFrame::decodeData(const u8 *buffer, size_t length)
{
	if (length != ADDRESS_FRAME_LENGTH) { // Check the length first
		throw J1939DecodeException(
			"[AdressClaimFrame::decodeData] Buffer length does "
			"not match the expected length. Buffer length:" +
			std::to_string(length) +
			". Expected length: " + std::to_string(ADDRESS_FRAME_LENGTH));
	}

	u32 idNumber = (buffer[0] | (buffer[1] << 8) | ((buffer[2] & 0x1F) << 16));

	u16 manufacturerCode = (((buffer[2] & 0xE0) >> 5) | (buffer[3] << 3));
	...
	mEcuName =
		EcuName(idNumber, manufacturerCode, ecuInstance, functionInstance,
				function, vehicleSystem, vehicleSystemInstance, industryGroup,
...
void AddressClaimFrame::encodeData(u8 *buffer, size_t length) const
{
	...
	buffer[4] = ((mEcuName.getEcuInstance() & ECU_INSTANCE_MASK) |
	...
	buffer[7] =
		(mEcuName.getVehicleSystemInstance() & VEHICLE_SYSTEM_INTERFACE_MASK) |
		((mEcuName.getIndustryGroup() & INDUSTRY_GROUP_MASK) << 4) |
...
std::string AddressClaimFrame::toString() const
{
	...
	sstr << "Manufacturer Code: "
		 << static_cast<u32>(mEcuName.getManufacturerCode()) << std::endl;
	sstr << "ECU Intance: " << static_cast<u32>(mEcuName.getEcuInstance())
		 << std::endl;
	sstr << "Function Instance: "
		 << static_cast<u32>(mEcuName.getFunctionInstance()) << std::endl;
	sstr << "Function: " << static_cast<u32>(mEcuName.getFunction())
		 << std::endl;
	sstr << "Vehicle System: " << static_cast<u32>(mEcuName.getVehicleSystem())
		 << std::endl;
	sstr << "Vehicle System Instance: "
		 << static_cast<u32>(mEcuName.getVehicleSystemInstance()) << std::endl;
	sstr << "Industry Group: " << static_cast<u32>(mEcuName.getIndustryGroup())
		 << std::endl;
	sstr << "Address Capable: "
		 << (mEcuName.isArbitraryAddressCapable() ? "yes" : "no") << std::endl;
