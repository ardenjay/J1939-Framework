...
class MyAddressClaimer : public AddressClaimer
{
  ...
  protected:
	void onSrcAddrChanged(u8 newAddr)
	{
		if (newAddr == J1939_INVALID_ADDRESS) {
			...
			std::cout << "New address obtained " << static_cast<u32>(newAddr)
					  << std::endl;
		}
	}
  ...
  public:
	MyAddressClaimer(const EcuName &name, const std::queue<u8> &preferred,
					 std::shared_ptr<Can::ICanSender> sender)
		: AddressClaimer(name, preferred), mSender(sender)
	{
	}
	~MyAddressClaimer() {}
};
...
int main(int argc, char **argv)
{
	...
	while (1) {
		c = getopt_long(argc, argv, "i:p:n:", long_options, NULL);
	...
	addresClaimer = std::unique_ptr<MyAddressClaimer>(
		new MyAddressClaimer(name, preferred, sender));
