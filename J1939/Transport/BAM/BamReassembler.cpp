...
void BamReassembler::reassemble(const BAMFragments &fragments, u8 **data,
								size_t &length)
{
	size_t offset = 0;
	...
	*data = new u8[length];
	...
	for (std::vector<TPDTFrame>::const_iterator iter =
			 fragments.getDtFrames().begin();
		 iter != fragments.getDtFrames().end(); iter++) {
		memcpy((void *)(*data + offset), (void *)iter->getData(),
			   J1939_MIN(length - offset, TP_DT_PACKET_SIZE));
...
bool BamReassembler::toBeHandled(const J1939Frame &frame) const
{
	...
	if (frame.getDstAddr() !=
		J1939_BROADCAST_ADDRESS) { // Does not have a broadcast address
		return false;
	}
	...
	case TP_CM_PGN: {
		...
		return (conn->getCtrlType() ==
				CTRL_TPCM_BAM); // If the ctrl type is not BAM, discard it.
	} break;
	case TP_DT_PGN: {
		return true;
	} break;
...
size_t BamReassembler::handleFrame(const J1939Frame &frame)
{
	...
	if (frame.getDstAddr() !=
		J1939_BROADCAST_ADDRESS) { // The frame does not have a broadcast
								   // address
	...
	case TP_DT_PGN: // Data reception
	{
		...
		if ((connFrame.getTotalPackets() >= sq) &&
			(mFragments[srcAddr].getLastSQ() + 1 == sq)) {
		...
		if (sq == connFrame.getTotalPackets()) { // Time to reassemble the
												 // packet

			u8 *fullData;
			size_t length;
			reassemble(mFragments[srcAddr], &fullData, length);
			u32 newId = (((connFrame.getPriority() & J1939_PRIORITY_MASK)
						  << J1939_PRIORITY_OFFSET) |
						 (connFrame.getDataPgn() << J1939_PGN_OFFSET) |
						 (connFrame.getSrcAddr() & J1939_SRC_ADDR_MASK));

			std::unique_ptr<J1939Frame> decodedFrame =
				J1939Factory::getInstance().getJ1939Frame(newId, fullData,
														  length);
			delete[] fullData;
