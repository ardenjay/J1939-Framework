...
namespace J1939
{
AddressClaimer::AddressClaimer(const EcuName &name,
							   const std::queue<u8> &preferred)
	: mEcuName(name), mPreferred(preferred)
{
	ASSERT(!preferred.empty());

	mThread = std::unique_ptr<std::thread>(
		new std::thread(&AddressClaimer::exec, this));
}
...
bool AddressClaimer::toBeHandled(const J1939Frame &frame)
{
	...
	// but it would take more time to check and it is not worth it (check
	// preferred address...)
	case ADDRESS_CLAIM_PGN: { // The object will check in its own thread whether
							  // this frame should be treated.
...
AddressClaimer::ClaimResult AddressClaimer::tryObtainAddress(u8 address)
{
	...
	do {
		...
		if (frame && frame->getPGN() == ADDRESS_CLAIM_PGN) {
			...
			if (rcvFrame->getSrcAddr() == addrClaim.getSrcAddr()) {
				if (addrClaim.getEcuName() < rcvFrame->getEcuName()) {
					// Win
					return CLAIM_RETRY;
				} else {
					// Lose
					// Check next preferred address if any
					return CLAIM_FAILED; 
				}
			} else {
				// No contending frame
		...
		while (!conflict) {
			...
			if (frame) {
				...
				case REQUEST_PGN: {
					...
					if ((reqFrame->getRequestPGN() == ADDRESS_CLAIM_PGN) &&
						((reqFrame->getDstAddr() == J1939_BROADCAST_ADDRESS) ||
						 (reqFrame->getDstAddr() == mCurrentSrc))) {
						// Send claim address
						AddressClaimFrame frame(mEcuName);
