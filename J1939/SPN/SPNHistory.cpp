...
void SPNHistory::addSample(const Utils::TimeStamp &timeStamp, const SPN &spn)
{
	...
	case SPN::SPN_NUMERIC: {
		...
		if (!mSamples.empty() &&
			mSamples.back().getNumeric() == spnNum->getFormattedValue())
	...
	case SPN::SPN_STATUS: {
		...
		if (!mSamples.empty() &&
			mSamples.back().getStatus() == spnStat->getValue())
			return; // Value did not change
...
std::vector<SPNHistory::Sample>
SPNHistory::getWindow(const Utils::TimeStamp &timeStamp, u32 milliseconds,
					  u32 samples) const
{
	...
	size_t pos = std::upper_bound(mSamples.begin(), mSamples.end(), sample) -
				 mSamples.begin();

	if (pos != 0)
		--pos; // Start with the sample whose timestamp is lower or equals to
	...
	while (current <= timeStamp) {
		...
		while (pos + 1 < mSamples.size() &&
			   current >= mSamples[pos + 1].getTimeStamp()) {
