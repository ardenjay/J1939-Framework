...
{
	if (mHelpers.empty()) {
		std::set<std::string> socketCanIfaces =
			Sockets::SocketCanHelper::getCanIfaces();

		for (auto iter = socketCanIfaces.begin(); iter != socketCanIfaces.end();
			 ++iter) {
		...
		std::set<std::string> peakCanIfaces =
			PeakCan::PeakCanHelper::getCanIfaces();

		for (auto iter = peakCanIfaces.begin(); iter != peakCanIfaces.end();
			 ++iter) {
